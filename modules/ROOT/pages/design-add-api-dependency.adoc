...
NOTE: If you select *Show in Files*, the *Files* section expands and the dependencies are shown under the `exchange_modules` section. The subsections correspond to the URI for the referenced dependency, which is its asset identifier, or GAV (group ID, asset ID, and version), in Exchange.
