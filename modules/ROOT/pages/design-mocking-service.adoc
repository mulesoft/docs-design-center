...
[NOTE]
====
In Design Center's API editor and visual API editor, the URI that the mocking service uses is public. However, in Exchange, the URI uses authentication and can be accessed only by Exchange. Therefore, the URI in Design Center is never the same as the URI in Exchange.
====
