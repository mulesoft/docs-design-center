...
Use the mocking service to mock HTTP requests via a public URI to an API that you are creating the specification for. The service returns responses (both HTTP status codes and example payloads) that are defined in the specification.
...
[NOTE]
====
The public URI that the mocking service uses for an API specfication in Design Center's API editor and visual API editor is also available in Exchange. Therefore, people who discover your API specification can try it out by turning on the mocking service there and sending requests.
====
