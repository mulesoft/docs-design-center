...
== Problems with RAML NamedExample fragments and examples in general
// APIMF-907
...
* In the fragment, ensure that each example consists of a name and a value, as in the fragment below that contains one example:
+
.fragment.yaml
----
myExample:
 ...
 post:
   body:
     application/json:
       properties:
         firstName: string
       examples: !include ex.yaml
----

Further suppose that the fragment also names the example, giving it the name `someFragmentName`:
