...
image:apid-helpful-text-editor-1.png["Example of a JSON response containing a person's first name.", <30%,30%>]

Pressing Control + Shift + Right Arrow again results in the following selection range:

image:apid-helpful-text-editor-2.png["A code snippet that demonstrates a JSON object with a key named firstname.", <30%,30%>]
...
image:apid-helpful-text-editor-3.png["A RAML code that defines a type and resource for a person.", <75%,75%>]
...
image:apid-helpful-text-editor-4.png["A code that shows the definition of a "person" type, including a description, properties, and a manager type.", <37%,37%>]

Click the icon and select *Extract to Declaration*, which results in the following two changes to the document:

image:apid-helpful-text-editor-5.png["Code showcasing RAML for a test refactoring project.", <37%,37%>]
