...
You can use API Designer's visual API editor to scaffold RAML 1.0, or OAS 2.0 or 3.0._x_ (JSON), API specifications by populating language-neutral forms.
...
* The right panel displays the RAML Viewer and the OAS Viewer, both of which are read-only. Click the tabs at the bottom of the panel to switch between them. These viewers display the current draft of the specification in either RAML 1.0 or OAS 3.0._x_. As you enter values in the forms that appear in the middle panel, the draft is updated with the values.
+
