...
You can use API Designer's visual API editor to scaffold RAML or OAS API specifications by populating language-neutral forms.
...
As you populate the forms, the visual API editor generates two API specifications: one in RAML 1.0, the other in OAS 2.0 (in JSON). Both are visible in a read-only pane on the right of the screen. If at any time you want to edit the specification directly or customize it (import RAML fragments, add a security scheme, and more), click *Edit RAML*  or *Edit OAS* to switch to the API editor. After you switch, however, you cannot switch back to the visual API editor to continue creating the API specification.
...
If you are satisfied with your API specification, you can publish it to Exchange in RAML. To publish, click the icon that is in the top-right corner of the visual API editor to open the *Publish API specification to Exchange* dialog. The icon looks like this:
