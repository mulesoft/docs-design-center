...
You can use API Designer's text editor to create API specifications directly in RAML 0.8, RAML 1.0, or OAS 2.0 (either in JSON or YAML). To do so, you must first create an API-specification project in which you can organize the files in the specification. You can save API-specification projects in Design Center, or you can save them in GitHub as repositories.

If you choose to save a project as a repository in GitHub and the MuleSoft app is not yet installed in your GitHub organization, follow the instructions in the section "Install the MuleSoft Application in to Use a GitHub Organization with Your Anypoint Platform Organization" in xref:design-storage-github.adoc[Using Repositories in GitHub to Store Your Projects].
...
.. In the *New Specification* section of the dialog that is opened, name your project.
+
