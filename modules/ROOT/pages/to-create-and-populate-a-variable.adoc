...
====
+
*Example based on the screenshot of the Custom Expression windo:* If you wanted to save the payload's error message into a variable, you would click `error_message`. The *Expression* field would then contain the expression `error_message`. You would then click *OK*.
+
...
. Click the canvas to close the card.
