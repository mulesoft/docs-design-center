...
*Result:* The canvas for your project is opened and the *Let's get started* wizard appears over it. If you have a simple workflow for which you need only a trigger and one other card, you can sequence the two cards with the help of this wizard. Otherwise, click *Go straight to the canvas*.
. Specify a trigger card and an action that causes the trigger to start the workflow.
