...
. The project pane
+
Click the top icon to list the project's flows, the configurations that are being used, and any global data types that have been defined. Click the bottom icon to list the project's dependencies.
+
...
. The canvas
+
Here is where you create a flow. In the top-left corner are the names of the project and the flow. The cards in a flow appear in the middle, in a sequence from left to right. The first card in a flow is the trigger card, which represents the connector and operation that you want to use to trigger the flow.
. The control bar
+
Lets you run the application for testing, as well as deploy the application to another environment.
