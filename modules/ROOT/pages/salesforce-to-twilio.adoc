...
You need to ensure that your application is triggered as soon as the status of an Opportunity in Salesforce is changed to ‘Closed Won’. To do this, you specify in Salesforce which fields to send and our criteria for triggering the application. You can specify all of this with an SOQL query. After you build the query, you can set up a Push Topic that can serve as the trigger for your MuleSoft application.

. Navigate to the xref::https://workbench.developerforce.com/query.php[SOQL query builder]. (You have to log in with your Salesforce credentials.)
...
Now, every time the query criterion is met, Salesforce will push an event into the `ClosedOpps` Push Topic.
...
A Mule application consists of one or more _flows_. According to the MuleSoft glossary, a flow is "a simple, flexible mechanism that enables orchestration of services using the sophisticated message flow capabilities of Mule runtime engine. Also referred to as service orchestration." In Flow Designer, what this means is a sequence of _cards_ that represent core components in Mule Runtime or the operations of connectors. (Cards can represent a couple of other things, too; however, for this tutorial, you need know only about those two.)
