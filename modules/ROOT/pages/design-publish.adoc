...
<5> *Asset name*: The name that Anypoint Exchange uses to identify the published project. The field is defaults to a name that you should typically use. 
<6> *Asset ID*: The ID that Anypoint Exchange uses to identify the published project. The field defaults to an ID that you should typically use.
+
...
NOTE: If you are using the non-Crowd version of Anypoint Platform and want to add the asset to API Manager while publishing to Exchange, select *Add to API Manager*. This functionality is not available in the Crowd version and instead you can link API instances in API Manager to assets in Exchange.
...
* If you are republishing the specification and select `In Development`, you can use the same number as a previously published in-development version of the API, but not the same number as a previously published stable version.
