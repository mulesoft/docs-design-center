...
*  the *Asset version* and *API version* fields are now pre-populated using the following criterion:
** If no version is defined in the API spec, the suggested asset version and API version fields are pre-filled with its default value: `1.0.0` and `v1`.
** If the version define in the API spec matches asset version field recommendations (e.g `1.0.3`), the value is pre-filled in the asset version field, and the API version field is pre-filled with a `v` and the major asset version (e.g `v1`).
...
** If the version defined in the API spec is a different string (e.g `7.4`), the value is pre-filled in the API version field, the asset version field is pre-filled with its default value: `1.0.0`.
