...
For example, suppose that you add a Database connector to a flow. When you create configuration Config 1 for it, you create Connection MySQL to specify the information that the your Mule app needs to connect to a MySQL database at runtime. Now, you want to test your Mule app against an Oracle database.
...
.For every configuration that you create for a connector, in the *Advanced* section you can specify information in addition to the connection for the configuration to use.
image::fd-config-advanced.png[The Advanced section of the Database connector]
<1> The text to click to open the *Advanced* section.
<2> The additional information that you can specify for the Database connector.
