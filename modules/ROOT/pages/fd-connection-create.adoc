...
<4> In the procedure explained in the next section, you click the *Add connection* button to create a connection.
<5> You return to the configuration dialog for the connector, so that you can specify values for fields in the *Advanced* section, if you need to do so.
...
You can also create a connection if you are editing a configuration or creating an additional configuration for a connector.

image::fd-edit-switch-configs.png[]
<1> When there is at least one configuration for a connector, the top of the connector card looks like this. You can edit the configuration or switch to a different configuration. If you edit the configuration, the configuration dialog is opened and you can switch connections or create a connection.
<2> If you select the option to switch to a different configuration, the top of the card changes to look like this. You can either select a different configuration, if one exists, or add a new configuration. If you add a new configuration, you can use an existing connection or create a new one.

== To Add a Connection to a Configuration of a Connector
