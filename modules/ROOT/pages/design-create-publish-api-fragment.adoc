...
The following list displays OAS 3.0 limitations you should consider when writing your API Fragments:

* The only valid fields at the document root level are: `openapi`, `info`, `components`, and `paths`.

* The only valid fields in the `info` object are `title` and `version`.
