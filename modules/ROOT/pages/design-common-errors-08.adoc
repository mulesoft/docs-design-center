...
When testing its new RAML parser (which was release on October 24, 2018), MuleSoft identified ten common errors in API specifications written in RAML 0.8. This topic explains the errors with examples and describes how to fix them. They are not listed in any particular order, such as frequency of occurrence in the sample of API specifications that MuleSoft tested.
