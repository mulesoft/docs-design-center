...
If you do not follow these rules, you will see the warning messages that are described in the next two sections.
...
You must use an `examples` (with an `s`) facet. For example, the following reference to `fragment.raml` generates the warning message:
...
==== Not naming named examples or including more than one example in a NamedExample fragment

If you see this warning message in the API editor, then a NamedExample fragment is missing a name or contains more than one named example:
----
