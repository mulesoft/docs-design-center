...
Casting to the declared data type is not done automatically. To illustrate the error, here is an invalid declaration of an enum:
...
The data type for the enum is `string`; however, the values are all integers and are not cast to `string` values automatically. Either the type is declared incorrectly and should be `integer`, or the enum values need to be in quotation marks.
