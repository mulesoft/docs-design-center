...
If a data type includes a `value` facet and your NamedExample for the type does not include the type's name, the parser assumes that `value` is the name of the type, as in this example:
...
To resolve the warning, simply add the name of the type:
...
To resolve the problem, you would need to split the fragment into two fragments:
----
#%RAML 1.0 NamedExample
Name1:
   a: "b"
