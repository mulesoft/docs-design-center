...
.Example authentication request to the mocking service from a third-party tool
```
$ curl -X POST \
...
.Example authentication sent by the mocking service in response to the request
```
...
.Example asset shared on Exchange
```
...
This asset contains this API specification:
...
. Example `GET` request that uses an authentication token
```
$ curl -X GET \
...
.Response from the mocking service
```
{"status":"OK"}
```
