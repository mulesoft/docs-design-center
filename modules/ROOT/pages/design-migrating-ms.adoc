...
If an API-specification project is older than January 10, 2019 and the mocking service was switched on before that date, follow these steps to migrate to version 2 of the mocking service:
...
If a person is in Exchange and trying to simulate a call to a method, that person is prompted by Exchange to supply OAuth 2.0 credentials before sending a request.
