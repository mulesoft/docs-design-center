...
+
The connection being used by the configuration is the connection that is selected when the Flow Designer opens the dialog.
.. Specify the required connection information.
.. Follow steps b through d for each configuration that is marked by a red circle.
