...
[IMPORTANT]
====
After a new version of Mule runtime is released, opening an existing Mule-app project causes the application to use that new version. The application is upgraded immediately, Flow Designer then restarting it in the Design environment only. If Flow Designer is unable to upgrade an application, the application is not restarted.
