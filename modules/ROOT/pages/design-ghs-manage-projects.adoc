= Manage Projects Synced with GitHub
...
Manage and interact with GitHub-synced projects in API Designer and Design Center. 
...
* In the Design Center *Projects* list, a project’s *Details* pane enables you to view both GitHub sync status and repository URL - useful for troubleshooting connectivity issues. The *Details* pane also enables you to disable GitHub sync (you can also disable GitHub sync via the *Publish* options menu).
...
NOTE: The *View in GitHub* button, *View Diff* button, and commit hash do not appear for non-GitHub-synced projects.
...
=== Determine if a Project is Synced with GitHub
The Design Center *Projects* page lists your specifications, fragments, and applications. Projects that are synced with GitHub contain the GitHub icon in the *Name* column, as demonstrated in the following example:
...
View GitHub sync status in a selected project’s *Details* pane.

The *Details* pane includes the following indicator:

image:apid-ghs-sync-status-indicator.png[20%,20%]

The indicator displays *Not Available* if you have not authorized the GitHub Sync application in your GitHub account, or if you are not a member of a GitHub Sync-authorized GitHub organization. In these cases, you can view the project in API Designer as read-only.  

See xref:design-ghs-install-authorize.adoc[Install and Authorize the MuleSoft GitHub Sync Application].

=== Disable GitHub Sync

Disable GitHub sync status in a selected project’s *Details* panel. 
