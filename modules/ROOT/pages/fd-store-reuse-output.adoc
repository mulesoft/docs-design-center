= Store a Value that is Based on the Output that has not yet been Passed from a Card (Flow Designer)
...
Normally, a card outputs data that is different from the data that it received as input, as depicted in the first flow in the diagram.
