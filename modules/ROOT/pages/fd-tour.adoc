...
.The cards in an example flow
image::FD-annotated-2.png[]

The cards in a flow each represent a core component, an operation of a connector, a module, or an API. In this example, the cards represent the following core components and connectors:
...
. The Send Message operationg of the Twilio connector

To learn how to create this flow, see xref::salesforce-to-twilio.adoc[Create Your First Mule Application with Flow Designer].

The cards are displayed in a sequence from left to right. The first card in a flow is the trigger card, which represents the connector and operation that you want to use to trigger the flow.
