= Scatter Message Elements into Different Parallel Processes, then Gather the Results
...
<1> Any card can pass messages to a Scatter-Gather card, and this card can be the trigger of the current flow or any other card in the flow.
<2> The Scatter-Gather card in included in a larger rectangle that is called the _scope_ of the card. Within this scope are two or more sets of card, each set called a _route_. The Scatter-Gather card processes all routes at the same time, and the routes are all independent of each other. Within a route, you can include any card, though it would not make sense to include an HTTP Listener because you don't want to hold up processing while waiting for an HTTP request. Each route can have one or more cards.
...
Pamella Fite,0028t000001J1LOQ3,pamella.fite@aigiant.com
...
Britt Lafferty,0028t000001J6HUT7,britt.lafferty@hoipolloi.com
