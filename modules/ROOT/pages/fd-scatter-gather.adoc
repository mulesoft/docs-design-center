...
With a Scatter-Gather component, you can send a message through more than one set of cards at a time, collect the results from each set, and pass those results (along with the original message) to the next card in a flow.
...
<1> Any card can pass messages to a Scatter-Gather component, and this card can be the trigger of the current flow or any other card in the flow.
<2> The card for the Scatter-Gather component in included in a larger rectangle that is called the _scope_ of the component. Within this scope are two or more sets of card, each set called a _route_. The Scatter-Gather component processes all routes at the same time, and the routes are all independent of each other. Within a route, you can include any card, though it would not make sense to include an HTTP Listener because you don't want to hold up processing while waiting for an HTTP request. Each route can have one or more cards.
...
Though this example uses an HTTP Listener card, you can send data to a Scatter-Gather component with any card.
