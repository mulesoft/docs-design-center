...
The payload that leaves the Scatter-Gather component schematically looks like this:
...
The message that the Transform card receives from the Set Variable card looks like this:
...
In this case, the first card in each route receives the same message, which schematically might look like this if you create two variables before the Scatter-Gather component:
...
Suppose that you create `var1` at the start of the first route, and you create `var2` at the start of the second route. Then, the second card in the first route receives a message that looks like this:
...
And the second card in the second route receives a message that looks like this:
