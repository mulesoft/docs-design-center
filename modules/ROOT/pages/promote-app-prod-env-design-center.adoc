...
[NOTE]
====
Any changes that you make to configurations are not applied to the configurations in your project. The changes are applied only to the app that you are deploying.

For example, suppose that your app uses ConfigA for a connector. In the project, the ConfigA uses the connection ConnA. When you deploy the app, you change ConfigA to use connection ConnB. However, ConfigA continues to use ConnA in your project.

Essentially, configurations are copied into an app when you are deploying that app. Changes that you make to configurations are applied only to the copies that are in the app, not to the originals that are in your project.

However, if you create a connection for a configuration when you deploy an app, Flow Designer takes these two actions:

* In the configuration that is copied into the app, the new connection is used.
* In the original configuration that is in the project, the new connection is not used, but it is added to the list of connections that are available for the configuration to use.
====
