...
For example, suppose that your application uses ConfigA for a connector. In the project, the ConfigA uses the connection ConnA. When you deploy the application, you change ConfigA to use connection ConnB. However, ConfigA continues to use ConnA in your project.

Essentially, configurations are copied into an application when you are deploying that application. Changes that you make to configurations are applied only to the copies that are in the application, not to the originals that are in your project.

However, if you create a connection for a configuration when you deploy an application, Flow Designer takes these two actions:

* In the configuration that is copied into the application, the new connection is used.
...
. If you want to change the way that any of the connectors in your application connect to systems, click *Customize connections for this deployment* to see a list of the configurations that the connectors in your application are using. Select a configuration on the left to edit it.
