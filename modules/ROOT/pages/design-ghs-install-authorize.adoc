...
. Under *Integrations*, disable the Github Integration.
...
WARNING: This action disables source syncing for all users and projects in your organization. If the integration is re-enabled, all past synchronizations are lost eveb when the repositories still exist on GitHub.
