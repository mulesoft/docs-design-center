...
<1> You can create a pool of connections for each type of connector.
<2> You can create a pool of configurations for each type of connector. Each configuration uses a single connection.
<3> Each instance of a type of connector uses a single configuration. For example, you might have two instances of the Salesforce connector in a Mule app. Each instance might use the same configuration or they might each use a different configuration. Typically, two configurations for a connector type each use a different connection.
