...
When you add to a Mule app a card that represents a connector, Flow Designer requires that you specify how your Mule app can connect to the system that the connector is for.

For example, if you add a card that represents a database connector, immediately after you select the type of operation that you want your app to perform in the database, Flow Designer opens the *Database Configuration* dialog. In that dialog, you can either select an existing connection to a database or create a connection.
