...
The right pane of the text editor in API Designer displays the documentation that is written into API specifications. For API specifications that are written in RAML or OAS 2.0, the ways that the documentation is rendered are identical. However, the documentation for API specifications that are written in OAS 3.0 can include multiple server definitions, as well as callbacks and links.
