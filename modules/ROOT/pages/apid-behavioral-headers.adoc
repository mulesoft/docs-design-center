...
+
`FIRST`:: The mocking service returns the first status code that is defined in a specification. For example, if the specification defines the status codes for a POST method in the order 500, 409, 400, 200, the mocking service will return the response for the 500 status code.
+
You can combine the use of MS2-Status-Code-Selection with these other behavioral headers:
+
* MS2-Error-Level The mocking service returns the first status code that falls within the range specified by 
