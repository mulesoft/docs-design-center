...
The MS2-Example header enables you to choose a specific response example if your endpoint defines more than for the same status code.

Using MS2-Example, if your RAML specification contains the following:
...
In this example, possible values of the header would be `one`, `two`, and `three`. Sending the value `one` as part of the request would return the properties `something` and `30`. Sending values other than `one`, `two`, and `three` return no results.
