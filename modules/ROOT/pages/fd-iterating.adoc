...
Counter Variable Name:: Name of the property that stores the index of the element that is being processed. For example, if a collection is `['a','b','c']`, the index of `a` is 0, of `b` is 1, and of `c` is 2. When the For-Each component is processing the second element, the value of the variable is 1.
