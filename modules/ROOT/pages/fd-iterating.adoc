...
When the processing of a collection stops because of an exception or timeout, Flow Designer displays a red circle on the card that represents the component in the flow. You can open the *Logs* pane to read the error message.
...
Root Message Variable Name:: Name of the property that stores the parent message. The parent is the complete, non-split message. // What is the root message variable name for? What does it store "the parent message" for? Is "parent message" the full collection?
...
Collection:: Specifies the expression that defines the collection of parts to be processed in parallel. By default, the component uses the payload passed to it by the previous card in the flow.
